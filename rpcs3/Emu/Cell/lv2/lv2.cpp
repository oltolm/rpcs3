...
bool lv2_obj::sleep(cpu_thread& cpu, const u64 timeout)
{
	...
	if (cpu.get_class() == thread_class::ppu)
	{
		if (u32 addr = static_cast<ppu_thread&>(cpu).res_notify)
		{
			...
			else if (auto notify_later_idx = std::find(std::begin(g_to_notify), std::end(g_to_notify), std::add_pointer_t<const void>{});
				notify_later_idx != std::end(g_to_notify))
			{
				*notify_later_idx = vm::reservation_notifier_notify(addr, true);

				if (notify_later_idx < std::end(g_to_notify) - 1)
				{
					// Null-terminate the list if it ends before last slot
					*(notify_later_idx + 1) = nullptr;
				}
			}
			else
			{
...
{
	auto notify_later_idx = std::find(std::begin(g_to_notify), std::end(g_to_notify), std::add_pointer_t<const void>{});
	...
	if (!g_pending && g_scheduler_ready)
	{
		...
		for (usz x = g_cfg.core.ppu_threads; target && x; target = target->next_ppu, x--)
		{
			if (target->state & cpu_flag::suspend)
			{
				...
				if (notify_later_idx == std::end(g_to_notify))
				{
	...
	if (notify_later_idx - 1 < std::end(g_to_notify) - 1)
	{
		// Null-terminate the list if it ends before last slot
		*notify_later_idx = nullptr;
	}
