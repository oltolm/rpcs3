...
void spu_cache::initialize(bool build_existing_cache)
{
	...
	if ((g_cfg.core.spu_decoder == spu_decoder_type::asmjit || g_cfg.core.spu_decoder == spu_decoder_type::llvm) && !func_list.empty())
	{
		...
		if (g_cfg.core.spu_debug)
		{
			...
			std::map<std::string_view, spu_program*> sorted;

			for (auto&& f : func_list)
			{
				// Interpret as a byte string
				std::string_view data = {reinterpret_cast<char*>(f.data.data()), f.data.size() * sizeof(u32)};
			...
			{
				{
					...
					sha1_starts(&ctx);
					sha1_update(&ctx, reinterpret_cast<const u8*>(bytes.data()), bytes.size());
					sha1_finish(&ctx, output);
					fmt::append(dump, "\n\t[%s] ", fmt::base57(output));
				}
...
spu_program spu_recompiler_base::analyse(const be_t<u32>* ls, u32 entry_point, std::map<u32, std::basic_string<char32_t>>* out_target_list)
{
	...
	while (lsa > 0 || limit < 0x40000)
	{
		...
		for (auto& pair : m_preds)
		{
			...
			std::basic_string<char32_t> workload;
	...
	struct rchcnt_loop_t
	{
		...
		std::basic_string<char32_t> origins;
